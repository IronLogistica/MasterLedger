...
@gl_bp.route("/libro-mastro")
@login_required
def libro_mastro():
    """Libro Mastro — TUTTI i mastrini insieme, uno dopo l'altro, invece di
    doverli aprire uno alla volta dal Piano dei Conti. Mostra solo i conti
    con almeno un movimento nel periodo (altrimenti la pagina sarebbe
    lunghissima di conti vuoti) — filtrabile per data."""
    date_from = request.args.get("date_from") or None
    date_to = request.args.get("date_to") or None
    ...
    righe = query.order_by(JournalLine.account_id, JournalEntry.doc_date.asc(), JournalEntry.id.asc()).all()

    mastrini = {}
    for r in righe:
        acc = r.account
        mastrini.setdefault(acc.id, {"account": acc, "movimenti": [], "saldo": Decimal("0")})
        m = mastrini[acc.id]
        dare = Decimal(str(r.dare or 0))
        avere = Decimal(str(r.avere or 0))
        m["saldo"] += dare - avere
        m["movimenti"].append({"entry": r.entry, "dare": dare, "avere": avere,
                               "saldo_progressivo": m["saldo"], "cost_center": r.cost_center})

    lista_mastrini = sorted(mastrini.values(), key=lambda m: m["account"].code)
    return render_template("gl/libro_mastro.html", mastrini=lista_mastrini,
